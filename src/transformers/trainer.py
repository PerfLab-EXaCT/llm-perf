...
class Trainer:
    ...
    def tokenizer(self, processing_class) -> None:
        #logger.warning("Trainer.tokenizer is now deprecated. You should use `Trainer.processing_class = processing_class` instead.")
        self.processing_class = processing_class
    ...
    def get_train_dataloader(self) -> DataLoader: #Returns a DataLoader class
        """
        ...
        if not isinstance(train_dataset, torch.utils.data.IterableDataset):
            ...
            dataloader_params["sampler"] = self._get_train_sampler() #!Multiple calls to get_train_sample
            # Set whether to drop the last incomplete batch
            dataloader_params["drop_last"] = self.args.dataloader_drop_last
            # Set the worker initialization function
    ...
    def get_eval_dataloader(self, eval_dataset: Optional[Union[str, Dataset]] = None) -> DataLoader:
        ...
        if not isinstance(eval_dataset, torch.utils.data.IterableDataset):
            dataloader_params["sampler"] = self._get_eval_sampler(eval_dataset) #!Multple calls to get_eval_sample
            dataloader_params["drop_last"] = self.args.dataloader_drop_last
            dataloader_params["prefetch_factor"] = self.args.dataloader_prefetch_factor
    ...
    def get_test_dataloader(self, test_dataset: Dataset) -> DataLoader:
        ...
        if not isinstance(test_dataset, torch.utils.data.IterableDataset):
            dataloader_params["sampler"] = self._get_eval_sampler(test_dataset) #!Multple calls to get_eval_sample
            dataloader_params["drop_last"] = self.args.dataloader_drop_last
            dataloader_params["prefetch_factor"] = self.args.dataloader_prefetch_factor
    ...
    def train(
        ...
        **kwargs,
        ...
        inner_training_loop = find_executable_batch_size( #Find the correct batch size in cause auto batch size is enabled
    ...
    def _inner_training_loop(
        ...
        if self.args.auto_find_batch_size: #?Auto find batch size?
        ...
        print('\033[93m' + f"Currently training with a batch size of: {self._train_batch_size}" + '\033[0m')
        # Data loader and number of training steps 

        train_dataloader = self.get_train_dataloader() #!LengthGroupedSampler created here
        ...
        # world_size is the total number of GPUs used
        ...
        if has_length(train_dataloader):
            ...
            else: #When max_steps is not set (Default -1)
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            steps_in_epoch = ( #Length of the dataset/batch size
                len(epoch_dataloader)
                if len_dataloader is not None
                else args.max_steps * args.gradient_accumulation_steps
            )

            #Multiple other callbacks throughout the training loop
            ...
            step = -1
            epoch_iterator = iter(epoch_dataloader) #!Create iterator for this dataset 
            ...
            #total_updates is the number of times parameters are updated in each epoch
            #total_updates = (length of dataset/batch size) // number of batches per update
            total_updates = steps_in_epoch // args.gradient_accumulation_steps + 1 #Floor updates + 1

            #!NEW
            #lengthList = [] #List of batches with dynamic padding
            lengthListNoPadding = [] #List of batches without padding
            total_nonzero_tokens = 0

            for _ in range(total_updates): #? Number of times parameters are updated in each epoch
                update_step += 1
                num_batches = args.gradient_accumulation_steps if update_step != (total_updates - 1) else remainder #(1)
                #Prints 1 because we update parameters every batch

                #?Gets the next 'num_batches' batches with 'batch size' sequences
                batch_samples, num_items_in_batch = self.get_batch_samples(epoch_iterator, num_batches) #!Iterator called here if iterator is empty
                #print(num_items_in_batch) prints None 

                #? Iterate through each 'num_batches' batch and update parameters
                for i, inputs in enumerate(batch_samples):

                    #!FUNCTION 
                    #batch_sequences = []
                    batch_sequences_no_padding = []

                    for number, sequence in enumerate(inputs['input_ids']):
                        ...
                        #batch_sequences.append(len(sequences))
                        batch_sequences_no_padding.append(length_no_padding_inputs)
                        total_nonzero_tokens += length_no_padding_inputs
                    
                    #lengthList.append(batch_sequences)
                    lengthListNoPadding.append(batch_sequences_no_padding)
                    #!END FUNCTION

                    step += 1
                    do_sync_step = (step + 1) % args.gradient_accumulation_steps == 0 or (step + 1) == steps_in_epoch #?True if it's the last step or iteration is gradient accumulation multiple
                    ...
                    with context():
                        tr_loss_step = self.training_step(model, inputs, num_items_in_batch) #!Calculate loss
                    ...
                    if do_sync_step:
                        ...
                        self._maybe_log_save_evaluate(tr_loss, grad_norm, model, trial, epoch, ignore_keys_for_eval) #!Evaluation done here if strategy = steps
                    else: #? If in this step we are not updating parameters
            ...
            self._maybe_log_save_evaluate(tr_loss, grad_norm, model, trial, epoch, ignore_keys_for_eval) #!Evaluation done here if strategy = epoch
        ...
        # return output of training to train function
        return TrainOutput(self.state.global_step, train_loss, metrics)
    
    #?END OF INNER_TRAINING_LOOP
    ...
    def get_batch_samples(self, epoch_iterator, num_batches):
        ...
        for _ in range(num_batches): # _ means the value is not used in the loop
            try:
                batch_samples += [next(epoch_iterator)]
            except StopIteration:
                break

        #batch_samples contains the input_ids that hold all the sequences in the batch (an array of sequences)
        #?How does next(epoch_iterator) know how many sequences to return? The DataLoader class

        # Keep default behavior the same
        if not self.model_accepts_loss_kwargs: #?Here is our case
