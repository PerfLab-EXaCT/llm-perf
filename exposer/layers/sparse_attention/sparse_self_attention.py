...
from exposer.layers.sparse_attention import SparsityConfig
...
class SparseSelfAttention(nn.Module):
    ...
    # add to cache
    def get_ops(self, L, device):
        if L not in SparseSelfAttention.ops:
            sparsity_layout = self.get_layout(L)
            sparse_dot_sdd_nt = matmul(sparsity_layout, self.sparsity_config.block, 'sdd', device, trans_a=False, trans_b=True)
            sparse_dot_dsd_nn = matmul(sparsity_layout, self.sparsity_config.block, 'dsd', device, trans_a=False, trans_b=False)
            sparse_softmax = softmax(sparsity_layout, self.sparsity_config.block, device)
    ...
    def forward(self, query, key, value, rpe=None, key_padding_mask=None, attn_mask=None):
        ...
        sparse_dot_sdd_nt, sparse_dot_dsd_nn, sparse_softmax = self.get_ops(tgt_len, query.device)
