...
from transformers.src.transformers.data.data_collator import default_data_collator
from transformers.src.transformers.testing_utils import require_accelerate, require_torch
from transformers.src.transformers.trainer_utils import RemoveColumnsCollator, find_executable_batch_size
from transformers.src.transformers.utils import is_torch_available
...
if is_torch_available():
    ...
    from transformers.src.transformers.modeling_outputs import SequenceClassifierOutput
    from transformers.src.transformers.tokenization_utils_base import BatchEncoding
...
class TrainerUtilsTest(unittest.TestCase):
    ...
    def test_group_by_length(self):
        ...
        indices = list(LengthGroupedSampler(4, lengths=lengths)) #!List of lengths is passed in
        #?Length of list = 100
        #?Batch size = 4
        #?MegaBatch size = 24

        #Flatten the list
        #indices = [x for xs in indices for x in xs] #?Not needed unless we yield in the class LengthGroupedSampler
        print(indices)
