...
from TransformerLibrary.src.transformers.data.data_collator import default_data_collator
from TransformerLibrary.src.transformers.testing_utils import require_accelerate, require_torch
from TransformerLibrary.src.transformers.trainer_utils import RemoveColumnsCollator, find_executable_batch_size
from TransformerLibrary.src.transformers.utils import is_torch_available
...
if is_torch_available():
    ...
    from TransformerLibrary.src.transformers.modeling_outputs import SequenceClassifierOutput
    from TransformerLibrary.src.transformers.tokenization_utils_base import BatchEncoding
    from TransformerLibrary.src.transformers.trainer_pt_utils import (
...
class TrainerUtilsTest(unittest.TestCase):
    ...
    def test_group_by_length(self):
        ...
        #indices = [x for xs in indices for x in xs] #?Not needed unless we use "yield" in the class LengthGroupedSampler
        print(indices)
        ...
        # Test that the indices should be from 0 to 99
        self.assertEqual(sorted(indices), list(range(100)))

        if test_NoShuffle:
            # Test that the biggest element should be first
            self.assertEqual(lengths[indices[0]], 50)
            self.assertEqual(lengths[indices[1]], 40)

            # Test that the lengths are sorted in descending order
            self.assertEqual(sorted(lengths, reverse=True), result)
