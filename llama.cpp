...
static void zeros(std::ofstream & file, size_t n) { //Appends zero 'n' times to tensor
    char zero = 0;
...
// !Profile Offloading Function Start
...
// !Profile Offloading Function End
...
static bool llm_load_tensors(
    ...
    auto inputStop = high_resolution_clock::now();
    std::cout << "Input Duration: " << duration_cast<microseconds>(inputStop - inputStart).count << std::endl;
    ...
    if (split_mode == LLAMA_SPLIT_MODE_LAYER) {
        ...
        // !Profile Repeating Layer1 Offload Start
        for (int64_t i = i_gpu_start; i < n_layer; ++i) {
            int layer_gpu = std::upper_bound(splits.begin(), splits.begin() + device_count, float(i - i_gpu_start)/act_gpu_layers) - splits.begin();
            model.buft_layer[i] = llama_default_buffer_type_offload(model, layer_gpu);
        }
        // !Profile Repeating Layer1 Offload End

        // assign the output layer
        // !Profile Output Layer1 Offload Start
        ...
        // !Profile Output Layer1 Offload End
    } else {
        ...
        // !Profile Repeating Layer2 Offload Start
        for (int64_t i = i_gpu_start; i < n_layer; ++i) {
            model.buft_layer[i] = {
                split_buft,
        ...
        // !Profile Repeating Layer2 Offload End
        // assign the output layer
        // !Profile Output Layer2 Offload Start
        if (n_gpu_layers > n_layer) {
            model.buft_output = {
                split_buft,
                llama_default_buffer_type_offload(model, main_gpu)
        ...
        // !Profile Output Layer2 Offload End
    }
    // !Profile Offloading End
...
static void llama_model_quantize_internal(const std::string & fname_inp, const std::string & fname_out, const llama_model_quantize_params * params) {
    ...
    // !Profile For Loop 1 Start
    for (int i = 0; i < ml.n_tensors; ++i) { //For each tensor in the function
        auto weight = ml.get_weight(i);
        struct ggml_tensor * tensor = weight->tensor;
        ...
        // write tensor data + padding LOOK INTO THIS CODE FOR ZERO PADDING
        // Change this to optimize LLM
        fout.write((const char *) new_data, new_size);

        // !Profile ZeroPadding Start
        zeros(fout, GGML_PAD(new_size, align) - new_size);
        // !Profile ZeroPadding End
    }
    // !Profile For Loop 1 End
