# Adapted from https://github.com/microsoft/DeepSpeed/tree/master/deepspeed/ops/sparse_attention
import torch
import random
...
class SparsityConfig:
    ...
    It contains shared property of different block-sparse sparsity patterns. 
    However, each class needs to extend it based on required property and functionality.
    """
...
class EmptySparsityConfig(SparsityConfig):
    """Configuration class to store `Empty` sparsity configuration.
    In reality, this is not sparse and no blocks are used. We keep it for the sake of comparison and comprehension.
    """

    def __init__(self, num_heads, block=16, different_layout_per_head=False):
        """Initialize the Empty Sparsity Pattern Config.
        In reality, this is not sparse and no blocks are used. We keep it for the sake of comparison and comprehension.

        Arguments:
             num_heads: required: an integer determining number of attention heads of the layer.
             seq_len: required: an integer determining number of attention heads of the layer.
             different_layout_per_head: optional: this is just for the sake of consistency with other sparsity formats; can ignore it for EmptySparsityConfig
        """

        super().__init__(num_heads, block, different_layout_per_head)

    def make_layout(self, seq_len):
        """Set 0 to all blocks of the layout meaning the pattern is empty; not sparse.

        Arguments:
             seq_len: required: an integer determining the underling sequence length; must be <= max sequence length

        Return:
             layout: a tensor of dimension (num_heads, num_blocks, num_blocks) containing sparsity layout of all head; for empty everything is 0
        """
...
class RandomSparsityConfig(SparsityConfig):
    """Configuration class to store `Random` sparsity configuration.
    This class extends parent class of `SparsityConfig` and customizes it for `Random` sparsity.
    """

    def __init__(self, num_heads, block=16, different_layout_per_head=False, sparsity=0.9):
        """Initialize the Random Sparsity Pattern Config.
        ...
        Arguments:
             ...
             block: optional: an integer determining the block size. Current implementation of sparse self-attention is based on blocked sparse matrices. In which this parameter defines size of such blocks, `Block X Block`.
             different_layout_per_head: optional: a boolean determining if each head should be assigned a different sparsity layout; default is false and this will be satisfied based on availability.
             sparsity: optional: a float determining the sparsity ratio; a number between 0 and 1; default is 0.9
        """
    ...
    def make_layout(self, seq_len):
        """Generates `Random` sparsity layout used by each head in the sparse attention.
        Arguments:
             seq_len: required: an integer determining number of attention heads of the layer.
        Return:
             layout: a tensor of dimension (num_heads, num_blocks, num_blocks) containing `Random` sparsity layout of all head
        """
        ...
        for h in range(0, self.num_layout_heads):
            for row in range(0, num_blocks):
                for col in range(0, num_blocks):
                    layout[h, row, col] = 1 if random.random() > self.sparsity else 0
        # set the first element to 1 to make sure the layout is not empty
        layout[:, 0, 0] = 1
        # tril the layout to make sure it is lower triangular
        for h in range(0, self.num_layout_heads):
            layout[h, :, :] = torch.tril(layout[h, :, :])
        # print nnz ratio
        nnz = layout.sum().item()
        print(f'Random Sparsity Layout: {nnz} / {num_blocks * num_blocks} = {nnz / (num_blocks * num_blocks)}')
...
class FixedSparsityConfig(SparsityConfig):
    ...
    def set_global_layout(self, h, layout):
        ...
        That means if a local window consists of 4 blocks and global attention size is one block, we use block #4 in each local window as global. 
        If we have different layout per head, then other heads will get #3, #2, and #1. 
        And if we have more heads (and different layout has set) than num of global attentions, multiple head may have same global attentions.
...
class VariableSparsityConfig(SparsityConfig):
    ...
    def set_random_layout(self, h, layout):
        ...
        for row in range(0, num_blocks):
            ...
            if (self.num_random_blocks > len(sample_range)):
                self.num_random_blocks = len(sample_range)
...
# class SparsityConfigFactory:
#     """Factory class to create sparsity config based on the given sparsity type.
#     """

#     @staticmethod
#     def get_sparsity_config(sparsity_type, num_heads, block, different_layout_per_head, **kwargs):
#         """Creates sparsity config based on the given sparsity type.

#         Arguments:
#              sparsity_type: required: a string determining the sparsity type. It can be `fixed`, `variable`, `bigbird`, `longformer`, or `bslongformer`.
#              num_heads: required: an integer determining number of attention heads of the layer.
#              block: optional: an integer determining the block size. Current implementation of sparse self-attention is based on blocked sparse matrices. In which this parameter defines size of such blocks, `Block X Block`.
#              different_layout_per_head: optional: a boolean determining if each head should be assigned a different sparsity layout; default is false and this will be satisfied based on availability.
#              kwargs: optional: a dictionary of parameters that are specific to the given sparsity type.

#         Return:
#              sparsity_config: a sparsity config object based on the given sparsity type
#         """

#         if (sparsity_type == 'fixed'):
#             return FixedSparsityConfig(num_heads, block, different_layout_per_head, **kwargs)
#         elif (sparsity_type == 'variable'):
#             return VariableSparsityConfig(num_heads, block, different_layout_per_head, **kwargs)
#         elif (sparsity_type == 'bigbird'):
#             return BigBirdSparsityConfig(num_heads, block, different_layout_per_head, **kwargs)
#         elif (sparsity_type == 'longformer'):
#             return LongformerSparsityConfig(num_heads, block, different_layout_per_head, **kwargs)
#         elif (sparsity_type == 'bslongformer'):
#             return LongformerSparsityConfig(num_heads, block, different_layout_per_head, **kwargs)
#         else:
#             raise ValueError(f'Unsupported sparsity type: {sparsity_type}!')

  
def generate_custom_layout(seq_len, head_configs):  
    """  
    Generate a custom layout for each head, where each head can have a different sparsity pattern.  
  
    Arguments:  
        seq_len: The length of the sequence for which the layout will be generated.  
        head_configs: A list of tuples, where each tuple contains the index of the head and  
                      an instance of one of the SparsityConfig derived classes.  
                      For example: [(0, DenseSparsityConfig(num_heads=1)),   
                                    (1, LocalSlidingWindowSparsityConfig(num_heads=1)), ...]  
  
    Returns:  
        layout: A tensor containing the custom sparsity layout for all heads.  
    """  
    if not head_configs:  
        raise ValueError("head_configs must contain at least one configuration.")  
  
    # Determine the total number of heads from the first configuration instance  
    total_num_heads = len(head_configs)
  
    # Initialize the layout tensor  
    num_blocks = seq_len // head_configs[0][1].block  
    layout = torch.zeros((total_num_heads, num_blocks, num_blocks), dtype=torch.int64)  
  
    # Apply each specified configuration to the corresponding head  
    for head_idx, config in head_configs:  
        # Generate the layout for the specific head using the provided configuration  
        head_layout = config.make_layout(seq_len)  
          
        # Assign the generated layout to the specific head in the overall layout  
        layout[head_idx] = head_layout[0]  # Assuming each config instance is for a single head  
