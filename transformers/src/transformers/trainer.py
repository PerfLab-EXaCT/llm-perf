...
class Trainer:
    ...
    def _inner_training_loop(
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            step = -1
            epoch_iterator = iter(epoch_dataloader) #!Iterator for this loop 
            ...
            for _ in range(total_updates): #? ?Number of times parameters are updated in each epoch
                ...
                #print("NEW")
                #print(batch_samples[0].input_ids.size(dim=1)) #TODO TYPE TENSOR

                #? Prints the sequence lengths of the batch
                if update_step != (total_updates - 1):
                    lengthList.append(batch_samples[0].input_ids.size(dim=1))

                #TODO: DOES BATCH_SAMPLES contain all the sequences in a batch? 
                ...
                for i, inputs in enumerate(batch_samples):
                    ...
                    step += 1
                    do_sync_step = (step + 1) % args.gradient_accumulation_steps == 0 or (step + 1) == steps_in_epoch #?Boolean 
